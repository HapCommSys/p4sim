    ...
    static TypeId tid =
        TypeId("ns3::P4SwitchNetDevice")
            ...
            .AddTraceSource("SwitchEvent",
                            "Emitted when a switch event occurs",
...
P4SwitchNetDevice::DoInitialize()
{
    ...
    case P4SWITCH_ARCH_V1MODEL:
        ...
        // Wire the PhyTxEnd-based port-readiness signal into the TM scheduler.
        SetPortTxDoneCallback(MakeCallback(&P4CoreV1model::PortTxComplete, m_v1modelSwitch));
        break;
                                     ...
                                     PacketType packetType)
{
    ...
    else if (m_channelType == P4CHANNELP2P)
    {
        ...
        NS_LOG_DEBUG("* Reconstructed Ethernet header: Source MAC: "
                     ...
                     << ", Protocol: 0x" << std::hex << eeh_1.GetLengthType() << std::dec);
...
P4SwitchNetDevice::AddBridgePort(Ptr<NetDevice> bridgePort)
{
    ...
    // Connect to the port's PhyTxEnd trace so that the TM scheduler learns
    // the exact moment the device finishes serialising a packet onto the wire.
    // TraceConnectWithoutContext requires the trace source to accept a
    // Callback<void, Ptr<const Packet>> – all three device types (Csma, P2P,
    // CustomP2PNetDevice) expose "PhyTxEnd" with that signature.
    bool ok = bridgePort->TraceConnectWithoutContext(
        "PhyTxEnd",
...
{
    uint32_t portsNum = GetNBridgePorts();
...
void
P4SwitchNetDevice::SetPortTxDoneCallback(Callback<void, uint32_t> cb)
{
    m_portTxDoneCallback = cb;
}

void
P4SwitchNetDevice::OnPortTxEnd(Ptr<NetDevice> srcDevice, Ptr<const Packet> /* packet */)
{
    if (m_portTxDoneCallback.IsNull())
    {
        return;
    }
    // Translate the NetDevice pointer back to its port index.
    uint32_t nPorts = GetNBridgePorts();
    for (uint32_t i = 0; i < nPorts; ++i)
    {
        if (GetBridgePort(i) == srcDevice)
        {
            NS_LOG_DEBUG("OnPortTxEnd: port " << i << " PhyTxEnd");
            m_portTxDoneCallback(i);
            return;
        }
    }
    NS_LOG_WARN("OnPortTxEnd: could not find port for device "
                << srcDevice->GetInstanceTypeId().GetName());
}
